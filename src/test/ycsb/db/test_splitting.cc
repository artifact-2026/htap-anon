...
namespace ycsbc {
    TestSplitting::TestSplitting(const std::string& dbname, const char *dbfilename, utils::Properties &props) {
        ...
        SetOptions(options_, dbfilename, false, levels, fieldcount);
    ...
    */
    int TestSplitting::Read(const std::string &table, const std::string &key, const std::set<std::string> *fields,
    ...
    void TestSplitting::SetOptions(rocksdb::Options& options_, const char *dbfilename, bool logging, int levels, int fieldcount)
    {
        if (!logging) {
