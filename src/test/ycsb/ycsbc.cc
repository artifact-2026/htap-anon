...
void runLoad(utils::Properties &props, int num_threads, ycsbc::DB *db,
             bool print_stats,
             std::vector<std::unique_ptr<ycsbc::CoreWorkload>> &wls);
void runXput(utils::Properties &props, int num_threads, ycsbc::DB *db,
             int run_time, bool print_stats,
             std::vector<std::unique_ptr<ycsbc::CoreWorkload>> &wls);

struct run_result DelegateClient(ycsbc::DB *db, ycsbc::CoreWorkload *wl,
                                 const int num_ops, bool is_loading) {
  std::cerr << "[Debug] DelegateClient started with db=" << db << " wl=" << wl
...
// workload spec's operation mix (read/update/insert/scan/rmw proportions)
// rather than issuing only reads or only inserts.  The throughputtype parameter
// has been removed; call client.DoTransaction() to honour NextOperation().
struct run_result DelegateForThroughput(ycsbc::DB *db, ycsbc::CoreWorkload *wl,
                                        int runTime) {
  ycsbc::Client client(*db, *wl);
...
int main(const int argc, const char *argv[]) {
  ...
  const bool throughput =
      utils::StrToBool(props.GetProperty("throughput", "false"));
  ...
  for (int t = 0; t < num_threads; ++t) {
    ...
    p_t.SetProperty(ycsbc::CoreWorkload::RECORD_COUNT_PROPERTY,
                    std::to_string(my_ops));
  ...
  if (run) {
    ...
    for (int i = 0; i < num_threads; ++i) {
      actual_ops.emplace_back(std::async(
          std::launch::async,
          [&, i, total_ops] { // <-- capture wls by reference (&), not by value
            return DelegateClient(db, wls[i].get(), total_ops / num_threads,
                                  /*is_loading=*/false);
          }));
    }
    ...
    for (auto &n : actual_ops) {
      ...
      sum += rres.oks;
      txn_latencies.insert(txn_latencies.end(), rres.exec_time.begin(),
                           rres.exec_time.end());
    }
    ...
    double latency_mean =
        std::accumulate(txn_latencies.begin(), txn_latencies.end(), 0.0) /
        txn_latencies.size();
    double latency_sq_sum = std::accumulate(
        txn_latencies.begin(), txn_latencies.end(), 0.0,
    ...
    auto idx = [&](double p) {
      return txn_latencies[std::min((size_t)std::floor(p * (n - 1)), n - 1)];
    };
    auto p50 = idx(0.50), p99 = idx(0.99), p25 = idx(0.25), p75 = idx(0.75),
         min = idx(0.0);

    printf("********** run result **********\n");
    printf("all operation records:%d  use time:%.3f s  IOPS:%.2f iops (%.2f "
           "us/op)\n\n",
    ...
    printf("rmw ops   :%7lu  use time:%7.3f s  IOPS:%7.2f iops (%.2f us/op)\n",
           ...
           1.0 * temp_cnt[ycsbc::READMODIFYWRITE] * 1e6 /
               temp_time[ycsbc::READMODIFYWRITE] * num_threads,
    ...
    for (unsigned int i = 0; i < runfilenames.size(); i++) {
      ...
      for (int i = 0; i < num_threads; ++i) {
        actual_ops.emplace_back(std::async(
            std::launch::async,
            [&, i,
             total_ops] { // <-- capture wls by reference (&), not by value
              return DelegateClient(db, wls[i].get(), total_ops / num_threads,
                                    /*is_loading=*/true);
            }));
      }
  ...
  for (int i = 0; i < num_threads; ++i) {
    actual_ops.emplace_back(std::async(
        ...
        [&, i, total_rds] { // <-- capture wls by reference (&), not by value
          return DelegateClient(db, wls[i].get(), total_rds / num_threads,
                                /*is_loading=*/true);
        }));
  }
  assert((int)actual_ops.size() == num_threads);
...
void runXput(utils::Properties &props, int num_threads, ycsbc::DB *db,
  ...
  for (int i = 0; i < num_threads; ++i) {
    throughput_ops.emplace_back(std::async(std::launch::async, [&, i] {
  ...
  double mean =
      std::accumulate(exec_times.begin() + skip, exec_times.end(), 0.0) /
      (exec_times.size() - skip);
  ...
  double mean_xput = std::accumulate(xputs.begin() + skip, xputs.end(), 0.0) /
                     (xputs.size() - skip);
  double sum_of_squares_xput =
      std::accumulate(xputs.begin() + skip, xputs.end(), 0.0,
  ...
  printf(
      "throughput mean:%lf  stddev: %lf, average latency: %lf, stddev: %lf\n",
  ...
  if (xput_window > 0) {
    ...
    if (!out) {
      ...
      out << "window_start_sec,avg_throughput,stddev_throughput,min_throughput,"
             "max_throughput\n";
      for (int w = skip; w + xput_window <= run_time_in_units;
           w += xput_window) {
        double wmean = 0.0;
        for (int k = w; k < w + xput_window; ++k)
          wmean += xputs[k];
        wmean /= xput_window;
        double wsq = 0.0;
        for (int k = w; k < w + xput_window; ++k)
          wsq += std::pow(double(xputs[k]) - wmean, 2);
        double wstddev = std::sqrt(wsq / xput_window);
        uint64_t wmin = *std::min_element(xputs.begin() + w,
                                          xputs.begin() + w + xput_window);
        uint64_t wmax = *std::max_element(xputs.begin() + w,
                                          xputs.begin() + w + xput_window);
        out << w << "," << wmean << "," << wstddev << "," << wmin << "," << wmax
            << "\n";
      }
      printf("Per-window throughput stats written to '%s' (window=%ds, "
             "skip=%ds)\n",
